...
class ControllerModuleReviews extends Controller {
	...
	private function editSettings() {
		...
		if (strcmp(VERSION,'1.5.1.3') == 0) {
			$set = $this->model_module_reviews->getSettingFixed($this->name);
		} else {
	...
	public function install() {
		...
		$this->load->model('setting/setting');
		$this->model_module_reviews->createTable();
	...
	public function uninstall() {
		$this->load->model("module/{$this->name}");
		$this->model_module_reviews->dropTable();
	}
	
	public function editQuestion() {
		$this->load->model("module/{$this->name}");
		$this->model_module_reviews->editQuestion($this->request->post['pq']);
	}
	 
	public function deleteQuestion() {
		$this->load->model("module/{$this->name}");
		$this->model_module_reviews->deleteQuestion($this->request->get['question_id']);
	}	
	...
	public function index() {
		...
		$this->data['pqQuestions'] = $this->model_module_reviews->getQuestions(array(), $sort);
		$pagination = new Pagination();
		$pagination->total = $total_questions;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/reviews', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
		...
		$this->template = "module/reviews.tpl";
		$this->children = array(
			'common/header',
			'common/footer'	
		);
	...
	public function update() {
		$sql = "SHOW CREATE TABLE `" . DB_PREFIX . "reviewsnew`";
		...
		if (strpos($result->row['Create Table'],'customer_language_id') === FALSE) {
			...
			$sql = "RENAME TABLE `" . DB_PREFIX . "reviewsnew` TO `" . DB_PREFIX . "reviewsnew_old`";
			$this->db->query($sql);
			echo($sql . "<br><br>");
			...
			$sql = "SELECT * FROM `" . DB_PREFIX . "reviewsnew_old`";
	        ...
	        foreach ($res->rows as $row)  {
		    	...
		    	$customer_language_id = (int)$this->model_module_reviews->getLangIdByCode($this->config->get('config_language'));
