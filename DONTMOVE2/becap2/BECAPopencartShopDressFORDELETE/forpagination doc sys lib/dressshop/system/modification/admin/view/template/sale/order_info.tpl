...
<div id="content">
 ...
 <div class="pull-right"><a onclick="return showPostForm();" class="btn btn-info" id="ru_post_form" href="#">Почтовые формы</a> &nbsp;</div> 
 <div class="pull-right"><a onclick="return showPostForm();" class="btn btn-info" id="ru_post_form" href="#">Почтовые формы</a> &nbsp;</div> 
  ...
  <div class="container-fluid">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <div data-order="<?php echo $order_id; ?>" class="add-comment" style="float:left; width: 50%;">
          <textarea id='new-comment-text'></textarea><br/>
          <input type="button" id='add-new-comment' value="Добавить комментарий">
        </div>

        <div data-order="<?php echo $order_id; ?>">
          Трек номер <input type="text" name="track" value="" />
          <input type="button" id='add-track' value="Отправить письмо">
        </div>
        <br/>
        <div style="clear: both;"></div>
        <div style="margin: 20px 0;">
          <label><input id="is_ordered" type="checkbox" name="is_ordered" value="1" <?php if($is_ordered) { ?> checked <?php } ?> /> Товар заказан у поставщика</label>
        </div>

        <div style="clear: both;"></div>
        <script type="text/javascript">
          $(function() {
            $('#is_ordered').on('change', function() {
              $.ajax({
                url: 'index.php?route=sale/order/setOrdered&token=<?php echo $token; ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                  order_id: <?php echo $order_id; ?>,
                  is_ordered: $(this).prop('checked') ? 1 : 0,
                },
            ...
            $('#add-track').click(function(){
                ...
                var $track = $(this).parent().find('input[name=track]');
                ...
                if(!track) {
                  ...
                  $.ajax({
                    url: 'index.php?route=sale/order/addTrack&token=<?php echo $token; ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                      order_id: orderId,
                    ...
                    success: function(json) {
                      if(json.success) {
                        addOrderHistory(json.comment, 15);
                      }
                    }
                  });
            });
          });
          function addOrderHistory(comment, order_status_id) {
              $.ajax({
                url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/history&order_id=<?php echo $order_id; ?>',
                type: 'post',
                dataType: 'json',
                data: 'order_status_id=' + order_status_id + '&notify=0&append=0&comment=' + encodeURIComponent(comment),
                beforeSend: function() {
                ...
                success: function(json) {
                  ...
                  if (json['success']) {
                    $('.nav-tabs a[href="#tab-history"').trigger('click');
        ...
        <div class="comment-wrapp">
          <?php if($admin_comments) { ?>
            <?php foreach ($admin_comments as $commentt) { ?>
              <div class="admin-comment">
                <span class='date'><?php echo $commentt['date']; ?> : <span class='text'><?php echo $commentt['text']; ?></span>
                 <span data-id='<?php echo $commentt['comment_id']; ?>' class='remove'>X</span></span>
              </div>
            <?php } ?>
          <?php } ?>
        </div>
         ...
         <script type="text/javascript">
            $('#add-new-comment').click(function(){
              var orderId = $(this).parent().attr('data-order');
              var text = $('#new-comment-text').val();
              $.ajax({
                url: 'index.php?route=sale/order/addComment&token=<?php echo $token; ?>',
                type: 'post',
                dataType: 'json',
                data: 'order_id=' + orderId + '&text=' + text,
                complete: function(json) {
                    location.reload();
                  }     
              });
            });
            $('.admin-comment .remove').click(function(){
              var commentId = $(this).attr('data-id');
              $.ajax({
                url: 'index.php?route=sale/order/removeComment&token=<?php echo $token; ?>',
                type: 'post',
                dataType: 'json',
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="tab-order">
            <table class="table table-bordered">
             ...
             <!--  <tr>
              ...
              <tr>
                ...
                <td><a href="<?php echo $customer; ?>" target="_blank"><?php echo $lastname; ?> <?php echo $firstname; ?>  <?php echo $fax; ?></a></td>
              </tr>
              <?php } else { ?>
              <tr>
                <td><?php echo $text_customer; ?></td>
                <td> <?php echo $lastname; ?> <?php echo $firstname; ?> <?php echo $fax; ?></td>
              </tr>
              <?php } ?>
              ...
              <tr>
                ...
                <td><a href="mailto:<?php echo $email; ?>"><?php echo $email; ?></a>
                    <?php if($duplic_count_email > 1) { ?>
                     <span style='font-size: 16px; color: red;'>С этого Email уже заказывали <?php echo $duplic_count_email - 1; ?> раз! Заказы: </span>
                    <?php foreach ($duplic_email_id as $llll) { ?>
                      <?php if($llll['id'] != $order_id) { ?>
                        <a href="<?php echo  $llll['link']; ?>">#<?php echo  $llll['id']; ?></a>
                      <?php } ?>
                    <?php } ?>
                <?php } ?>
                </td>
              ...
              <tr>
                ...
                <td>
                 <?php if($duplic_count > 1) { ?>
                     <span style='font-size: 16px; color: red;'>С этого IP(<?php echo $ip-1; ?>) уже заказывали <?php echo $duplic_count-1; ?> раз! Заказы: </span>
                    <?php foreach ($duplic_ip_id as $lll) { ?>
                    	<?php if($lll['id'] != $order_id) { ?>
                      		<a href="<?php echo  $lll['link']; ?>">#<?php echo  $lll['id']; ?></a>
                    	<?php } ?>
                	<?php } ?>
                <?php } else { ?>
                 <?php echo $ip; ?>
              ...
              <?php } ?>
              <?php if ($user_agent) { ?>
               <?php if($duplic_count_agent > 1 && $duplic_count_cookie > 1 && $duplic_count > 1) { ?>
                  <tr>
                    <td>Браузер</td>
                    <td>
                        <span style='font-size: 16px; color: red;'>С этого браузера уже заказывали <?php echo $duplic_count_agent-1; ?> раз! Заказы: </span>
                      <?php foreach ($duplic_agent_id as $lllll) { ?>
                      	<?php if($lllll['id'] != $order_id) { ?>
    	                    <a href="<?php echo  $lllll['link']; ?>">#<?php echo  $lllll['id']; ?></a>
	                    <?php } ?>
                    <?php } ?>
                    </td>
                  </tr>
                <?php } ?>              
              <?php } ?>
                <?php if($duplic_count_cookie > 1 && $duplic_count > 1) { ?>
                  <tr>
                    <td>Cookie</td>
                    <td>
                        <span style='font-size: 16px; color: red;'>С одинаковыми cookies уже заказывали <?php echo $duplic_count_cookie-1; ?> раз! Заказы: </span>
                      <?php foreach ($duplic_cookie_id as $lllll) { ?>
                      	<?php if($lllll['id'] != $order_id) { ?>
                        	<a href="<?php echo  $lllll['link']; ?>">#<?php echo  $lllll['id']; ?></a>
                    	<?php } ?>
                    <?php } ?>
            ...
            <table class="table table-bordered">
...
</td><td class="text-left">Производитель
</td><td class="text-left">Производитель
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-left"><img src="<?php echo $product['image']; ?>"/><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a>
                  <b>
...
</td><td class="text-left"><?php echo $product['mf_name']; ?>
</td><td class="text-left"><?php echo $product['mf_name']; ?>
                  <td class="text-left"><?php echo $product['model']; ?></td>
                  <td class="text-right"><b><?php echo $product['quantity']; ?></b></td>
          ...
          <div class="tab-pane" id="tab-shipping">
            <table class="table table-bordered">
              ...
              <tr>
                <td>Идентификатор отправления (трек-номер):</td>
...
$('#set_track_no').on('click', function() {
	$.ajax({
		...
		complete: function() {
			...
			$('#tab-shipping').prepend('<div class="alert alert-success"><i class="fa fa-exclamation-circle"></i> Идентификатор отправления сохранен! <button data-dismiss="alert" class="close" type="button">×</button></div>');
		},
		success: function(html) {
		}
	});
