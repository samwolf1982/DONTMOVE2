...
class ControllerModuleMegaFilter extends Controller {
    ...
    public function index($setting) {
        ...
        if ($category_id) {
            ...
            if($category) {
                ...
                if($category['parent_id'] > 0) {
                    ...
                    $parent_category['name'] = $name_prefix . $parent_category['name'];

                    $results[] = $parent_category;

                    $name_prefix = '-- ';
                    foreach($this->model_catalog_category->getCategories($parent_category['category_id']) as $result) {
                        $result['name'] = $name_prefix . $result['name'];
                        $results[] = $result;

                        if($result['category_id'] == $category_id) {
                            $has_childs = true;
                            $results2 = $this->model_catalog_category->getCategories($category_id);
                            foreach($results2 as $result22) {
                                $result22['name'] = '--- ' . $result22['name'];
                                $results[] = $result22;
                            }
                        }
                    }
                ...
                foreach ($results as $result) {
                    ...
                    $data['categories'][] = array(
                        'selected' => $category_id == $result['category_id'],
                        'name' => $result['name'],
        ...
        $cat_path = isset($this->request->get['path']) ? $this->request->get['path'] : '';
        ...
        if ($mijo_shop) {
            ...
            MijoShop::getClass('base')->addHeader(JPATH_MIJOSHOP_OC . '/catalog/view/javascript/mf/mega_filter.js?222', false);
            ...
            $this->document->addScript('catalog/view/javascript/mf/mega_filter.js?v1' . $data['_v']);
