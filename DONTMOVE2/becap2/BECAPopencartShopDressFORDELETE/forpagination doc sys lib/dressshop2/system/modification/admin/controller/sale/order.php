...
$data['is_ordered'] = $order_info['is_ordered'];
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['payment_method'] = $order_info['payment_method'];
			...
			$this->load->model("sale/comment");

			$data['admin_comments'] = $this->model_sale_comment->getCommentsByOrderId($order_id);


			$count_dup = $this->model_sale_order->getCountOfOrdersByIp($order_info['ip']);
			$data['duplic_count'] = $count_dup[0]['count'];
			$data['duplic_ip_id'] = array();
			$ord_dup_ip = $this->model_sale_order->getOrdersIdByIp($order_info['ip']);
			foreach ($ord_dup_ip as $dupp) {
				$data['duplic_ip_id'][] = array(
					'id' 	=> $dupp['order_id'],
					'link'  => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'], 'SSL') . '&order_id=' . $dupp['order_id']
				);
			}


			$count_dup = $this->model_sale_order->getCountOfOrdersByEmail($order_info['email']);
			$data['duplic_count_email'] = $count_dup[0]['count'];
			$data['duplic_email_id'] = array();
			$ord_dup_ip = $this->model_sale_order->getOrdersIdByEmail($order_info['email']);
			foreach ($ord_dup_ip as $dupp) {
				$data['duplic_email_id'][] = array(
					'id' 	=> $dupp['order_id'],
					'link'  => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'], 'SSL') . '&order_id=' . $dupp['order_id']
			...
			$count_dup = $this->model_sale_order->getCountOfOrdersByCookie($order_info['cookie']);
			$data['duplic_count_cookie'] = $count_dup[0]['count'];
			$data['duplic_cookie_id'] = array();
			$ord_dup_cookie = $this->model_sale_order->getOrdersIdByCookie($order_info['cookie']);
			foreach ($ord_dup_cookie as $dupp) {
				$data['duplic_cookie_id'][] = array(
					'id' 	=> $dupp['order_id'],
					'link'  => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'], 'SSL') . '&order_id=' . $dupp['order_id']
				);
			...
			$count_dup = $this->model_sale_order->getCountOfOrdersByUserAgent($order_info['user_agent']);
			$data['duplic_count_agent'] = $count_dup[0]['count'];
			$data['duplic_agent_id'] = array();
			$ord_dup_ip = $this->model_sale_order->getOrdersIdByUserAgent($order_info['user_agent']);
			foreach ($ord_dup_ip as $dupp) {
				$data['duplic_agent_id'][] = array(
					'id' 	=> $dupp['order_id'],
					'link'  => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'], 'SSL') . '&order_id=' . $dupp['order_id']
	...
	public function history() {
		...
		foreach ($results as $result) {
			$data['histories'][] = array(
				'notify'     => $result['notify'] || strpos($result['comment'], 'www.pochta.ru') !== false  ? $this->language->get('text_yes') : $this->language->get('text_no'),
				'status'     => $result['status'],
	...
	public function invoice() {
                  ...
                  $data['admin_comments'] = $this->model_sale_comment->getCommentsByOrderId($this->request->get['order_id']);
	...
	public function addComment() {
		...
		$this->load->model('sale/comment');
		$this->model_sale_comment->addComment($data);

	}

	public function removeComment() {
		$this->load->model('sale/comment');
		$this->model_sale_comment->deleteComment($this->request->post['comment_id']);
	}
            ...
            public function addTrack() {
                ...
                if(!$track || !$order_id || !$order_info) {
                    ...
                    if ($order_info['email']) {
                        ...
                        $message = "У клиента ".$order_info['firstname'] . ' ' . $order_info['lastname'] . ' нет почты';
                    }
	...
	public function api() {
		...
		if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
			...
			if (substr($url, 0, 5) == 'https') {
				curl_setopt($curl, CURLOPT_PORT, 443);
			}//var_dump($url . 'index.php?route=' . $this->request->get['api'] . ($url_data ? '&' . http_build_query($url_data) : ''));
