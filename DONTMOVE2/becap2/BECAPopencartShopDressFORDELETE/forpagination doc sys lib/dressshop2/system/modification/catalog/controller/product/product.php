...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				$data['small'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_compare_width'), $this->config->get('config_image_compare_height'));
			} else {
				$data['small'] = '';
			}
			...
			foreach ($results as $result) {
                ...
                'small' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_compare_width'), $this->config->get('config_image_compare_height')),
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[] = array(
							...
							'price'                   => $price,
							'price0'                  => (float)$option_value['price'],
				...
				$data['options'][] = array(
					...
					'required'             => $option['required']
                        ...
                        //if one size or less, do not display it	
			if (count($option['product_option_id'])<=1) $option['required'] = false;
        ...
        $images = $this->model_catalog_product->getProductImagesForProducts();
          ...
          if (isset($category_info['name'])) {
            ...
            $data['liked_text'] = sprintf('Отложили <span>%s</span> %s', $data['liked'], $this->declOfNum($data['liked'], array('клиент', 'клиентов', 'клиента')));
          
            $this->document->addScript('catalog/view/javascript/fotorama.js');
            $this->document->addStyle(HTTP_THEME . 'css/fotorama.css');
