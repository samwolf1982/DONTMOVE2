...
class ControllerModuleReviews extends Controller {
	...
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/reviews_sidebar.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/module/reviews_sidebar.tpl';
		} else {
			$this->template = 'default/template/module/reviews_sidebar.tpl';
		}		
		
		$this->data['productquestion_conf_maxlen'] = $this->config->get('reviews_conf_maxlen');
		$this->data['pqsName'] = $this->customer->getFirstname();
		$this->data['pqsEmail'] = $this->customer->getEmail();
		
		$questions_per_page = $this->config->get('reviews_conf_qpp');
		...
		$langId = $this->model_module_reviews->getLangIdByCode($this->customer->session->data['language']);
		
		$this->data['pqQuestions'] = $this->model_module_reviews->getQuestions(
			array(
				'language_id' => $langId,
				'display' => 1,
				'product_id' => 0
		...
		$total_questions = $this->model_module_reviews->getQuestionCount(
			array(
				'language_id' => $langId,
				'display' => 1,
				'product_id' => 0
                ...
                foreach ($this->data['pqQuestions'] as &$review) {
                    if ($review['product_id']) {
                        $this->load->model('catalog/product');
                        $product_info = $this->model_catalog_product->getProduct($review['product_id']);
                        
                        if ($product_info['image']) {
                                $review['popup'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'));
                        } else {
                                $review['popup'] = '';
                        }
                        $review['href'] = $this->url->link('product/product', 'product_id=' . $product_info['product_id']);                        
                        $review['pname'] = html_entity_decode($product_info['name']);
                    }
                    else {
                        $review['popup'] = '';
                        $review['href'] = '';                        
	...
	public function getForm() {
		$language_id = $this->model_module_reviews->getLangIdByCode($this->customer->session->data['language']);
			...
			array(
                ...
                foreach ($this->data['pqQuestions'] as &$review) {
                    if ($review['product_id']) {
                        ...
                        if ($review['image']) {
                                $review['popup'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
                        } else {
                                $review['popup'] = '';
                        }
                        $review['href'] = $this->url->link('product/product', 'product_id=' . $product_info['product_id']);                        
	...
	public function submitProductQuestion() {
		...
		if (!isset($this->request->post['captcha']) || empty($question['question_text']) || empty($question['name']) || empty($question['email'])) {
			...
			if (mb_strlen($question['question_text'],'UTF-8') > $this->config->get('reviews_conf_maxlen')
			&& $this->config->get('reviews_conf_maxlen') > 0) {
	...
	public function questions() {
		...
		$pagination->limit = $questions_per_page; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/reviews/questions', '&page={page}');
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/reviews/questions'),
       		'separator' => ' » '
   		);				
