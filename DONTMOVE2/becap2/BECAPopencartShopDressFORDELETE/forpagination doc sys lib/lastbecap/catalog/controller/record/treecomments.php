...
class ControllerRecordTreeComments extends Controller
{
		...
		*/
		...
		if (isset($this->request->get['product_id']) || isset($this->request->get['record_id']) || isset($this->data['thislist']['recordid'])) {
			...
			if (count($this->data['comments']) > 0) {
				...
				if ($this->data['cmswidget'] == 2) {
						$temp_comments = array();
						foreach ($mycomments as $num => $result) {
							/*if ($result["date_releazed"] == "0000-00-00 00:00:00")*/ {
								if ($result["parent_id"]) {
									if (isset($temp_comments[$result["parent_id"]]))
									$temp_comments[$result["parent_id"]]["admin_comments"][] = $result;
									//unset($this->data['comments'][$num]);
								}
								else $temp_comments[$result["comment_id"]] = $result;
								//if ($result["date_releazed"] != "0000-00-00 00:00:00") unset($this->data['comments'][$num]);
							}
						}
						$mycomments = $temp_comments;
						$comment_total = count($mycomments);
				}
				foreach ($mycomments as $num => $result) {
	...
	public function write()
	{
		...
		if ($this->request->post['af']['email_comment']=='' || !preg_match("|^[-0-9a-z_\.]+@[-0-9a-z_^\.]+\.[a-z]{2,6}$|i", $this->request->post['af']['email_comment'])) {
	              		$json['error'] = $this->language->get('text_error_email');
						$html = "<script>var wdata = new Array()
								wdata['code'] 	 = 'error'
								wdata['message'] = '".$this->language->get('text_error_email')."'</script>";
