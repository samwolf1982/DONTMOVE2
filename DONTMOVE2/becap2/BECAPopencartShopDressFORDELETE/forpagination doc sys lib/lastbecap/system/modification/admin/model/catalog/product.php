...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_recurrings'])) {
			foreach ($data['product_recurrings'] as $recurring) {
				if (isset($recurring['recurring_id']))
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.*,pd.*,mf.name mf_name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) left join " . DB_PREFIX . "manufacturer mf ON (p.manufacturer_id = mf.manufacturer_id) WHERE pd.language_id = " . (int)$this->config->get('config_language_id') .' ';
