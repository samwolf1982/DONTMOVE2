...
class ModelCatalogProduct extends Model {
	...
	//ocmod
        public function updateLiked($product_id, $is_plus = true) {
			if($is_plus) {
				$this->db->query("UPDATE " . DB_PREFIX . "product SET liked = (liked + 1) WHERE product_id = '" . (int)$product_id . "'");
			}
			else {
				$this->db->query("UPDATE " . DB_PREFIX . "product SET liked = (liked - 1) WHERE product_id = '" . (int)$product_id . "' AND liked >= 1");
			}


	}

	public function updateAddedtocart($product_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET added_to_cart = (added_to_cart + 1) WHERE product_id = '" . (int)$product_id . "'");
	}
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
            ...
            if (!empty($data['days_new_product']))
            $sql .= " AND p.date_available > NOW() - INTERVAL " . $data['days_new_product'] . " DAY";
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            $sql .= $this->addQueryForSearchProductsRequest();
    ...
    // Вытаскиваем изображения для товара в категории
    public function getProductImagesForProducts() {
        $images = array();
		$query = $this->db->query("SELECT p.product_id, i.* FROM " . DB_PREFIX . "product_image as i LEFT JOIN " . DB_PREFIX . "product as p ON p.product_id = i.product_id ORDER BY i.sort_order ASC");
        if(count($query)){
            foreach($query->rows as $rows){
                $description[$rows['product_id']][] = array(
                    'image' => $rows['image']
                );
            }
        }
	...
	public function getProfile($product_id, $recurring_id) {
		return $this->db->query("SELECT * FROM `" . DB_PREFIX . "recurring` `p` JOIN `" . DB_PREFIX . "product_recurring` `pp` ON `pp`.`recurring_id` = `p`.`recurring_id` AND `pp`.`product_id` = " . (int)$product_id . " WHERE `pp`.`recurring_id` = " . (int)$recurring_id . " AND `status` = 1 AND `pp`.`customer_group_id` = " . (int)$this->config->get('config_customer_group_id'))->row;
                ...
                private function addQueryForSearchProductsRequest($table_prefix = 'p') {
                    return ' AND '. $table_prefix . '.stock_status_id != 5';
                }
